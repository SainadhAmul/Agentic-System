...
def _find_gateway_v7_dir() -> Path:
    candidates = [
        Path(__file__).resolve().parent / "llm_gatewayV7",
        Path(__file__).resolve().parents[1] / "llm_gatewayV7",
        Path(__file__).resolve().parents[2] / "llm_gatewayV7",
    ]
    for c in candidates:
        if c.exists():
            return c
...
GATEWAY_V7_DIR = _find_gateway_v7_dir()
