...
async def _crawl4ai_fetch(url: str) -> dict:
    ...
    # Use a silent logger so rich doesn't try to write Unicode characters (like arrows)
    # to legacy Windows console streams with cp1252 charmap encoding.
    silent_logger = AsyncLogger(verbose=False, log_level=LogLevel.CRITICAL)
    ...
    os.dup2(2, 1)
    try:
        async with AsyncWebCrawler(verbose=False, logger=silent_logger) as crawler:
